...
class Game {
   ...
   *
   * @param {Object} gameID for the game id
   * @param {Object} io for the socket
   *
   * @returns {void}
   */
...
Game.prototype._findPlayerIndexBySocket = function (thisPlayer) {
  let playerIndex = -1;
  each(this.players, (player, index) => {
...
Game.prototype.pickCards = function (thisCardArray, thisPlayer) {
  ...
  if (this.state === 'waiting for players to pick') {
    ...
    if (playerIndex !== -1) {
      ...
      each(this.table, (pickedSet, index) => {
...
Game.prototype.pickWinning = function (thisCard, thisPlayer, autopicked) {
  ...
  if ((playerIndex === this.czar || autopicked) && this.state === 'waiting for czar to decide') {
    let cardIndex = -1;
    each(this.table, (winningSet, index) => {
...
Game.prototype.continue = (self) => {
  if (self.state === 'black card') {
    self.stateChoosing(self);
  }
};

Game.prototype.getCzar = (self) => {
  self.state = 'black card';
  self.table = [];
